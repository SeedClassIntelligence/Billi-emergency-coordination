...
export default function AiAnalysisPanel({ incident, onActivate }: AiAnalysisPanelProps) {
  ...
  return (
    <div id="ai_coordination_panel" className="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 text-slate-850 shadow-sm relative overflow-hidden">
      ...
      <div className="flex justify-between items-center pb-3 border-b border-slate-150">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-slate-850 text-sm tracking-wide font-display uppercase">GEMINI EMERGENCY ANALYTICS</h3>
        </div>

        <button
          onClick={() => runAiAnalysis(targetLang)}
          disabled={loading}
          className="bg-slate-50 hover:bg-slate-100 border border-slate-200 disabled:opacity-50 p-1.5 rounded-lg flex items-center justify-center text-slate-500 hover:text-slate-800 cursor-pointer transition-colors"
          title="Refresh AI Extraction Summary"
        >
        ...
        {loading && !aiResult ? (
          ...
          >
            ...
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-2 relative">
              <span className="absolute top-2.5 right-3 text-[9px] font-mono text-blue-600 flex items-center gap-1 font-bold">
                <Sparkles className="w-3.5 h-3.5" />
                <span>REAL-TIME INSIGHT</span>
              </span>
              <p className="text-xs font-mono text-slate-450 uppercase tracking-widest block font-bold">TACTICAL TIMELINE REPORT:</p>
            ...
            {targetLang !== "none" && aiResult.translation && (
              <motion.div 
                ...
                className="bg-blue-50/50 border border-blue-200/60 p-4 rounded-xl space-y-1.5"
              >
            ...
            <div className="space-y-2">
              <p className="text-xs font-mono text-slate-450 uppercase tracking-widest font-bold">GEMINI EXTRACTED KEY OBSERVATIONS:</p>
              <ul className="space-y-2 text-xs text-slate-700">
                {aiResult.keyObservations.map((obs, idx) => (
                  <li key={idx} className="flex gap-2.5 items-start leading-relaxed">
                    <span className="text-blue-600 font-bold font-mono mt-0.5">✓</span>
            ...
            {aiResult.responderDirectives && aiResult.responderDirectives.length > 0 && (
              <div className="bg-emerald-50/70 border border-emerald-200/80 p-4 rounded-xl space-y-2">
                <p className="text-xs font-mono text-emerald-900 uppercase tracking-widest block font-bold flex items-center gap-1.5">
                ...
                <ul className="space-y-2 text-xs text-emerald-950 font-sans">
                  {aiResult.responderDirectives.map((dir, idx) => (
                    <li key={idx} className="flex gap-2 items-start font-medium leading-relaxed">
                      <span className="bg-emerald-600 text-white font-mono text-[9px] font-bold px-1.5 py-0.5 rounded mt-0.5 flex-shrink-0">
                        {idx + 1}
                      </span>
                      <span>{dir}</span>
                    </li>
