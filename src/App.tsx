  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 antialiased flex flex-col justify-between selection:bg-blue-500/30">
      ...
      <header className="bg-slate-900 border-b border-slate-800 text-white px-4 sm:px-6 py-3 flex flex-col md:flex-row justify-between items-center gap-3 sticky top-0 z-30 shadow-lg backdrop-blur-md bg-slate-900/95">
        ...
        <div className="flex items-center gap-1.5 bg-slate-950 p-1.5 rounded-2xl border border-slate-800 text-xs font-mono font-bold overflow-x-auto max-w-full">
          <button
          ...
          >
            <span>1. Child</span>
            <span className={`w-2 h-2 rounded-full ${incident ? "bg-red-500 animate-ping" : "bg-emerald-500"}`} />
          </button>
          ...
          >
            ...
            {incident?.contacts.find(c => c.id === "contact_mom")?.respondStatus !== "none" ? (
          ...
          >
            <span>3. Responder</span>
            {incident?.contacts.find(c => c.id === "contact_officer")?.respondStatus !== "none" ? (
          ...
          >
            <span>4. Devices</span>
            <span className={`w-2 h-2 rounded-full ${incident ? "bg-red-500 animate-pulse" : "bg-blue-500 animate-pulse"}`} />
          </button>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6 z-10">
        ...
        {/* --- LIVE SIMULATION SCENARIO EXPLAINER BANNER --- */}
        {incident && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-blue-800/60 p-4 rounded-2xl text-white text-left space-y-3 shadow-md"
          >
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-blue-900/60 pb-2.5">
              <div className="flex items-center gap-2">
                <span className="text-xl">⚡</span>
                <div>
                  <span className="text-[9.5px] font-mono font-bold text-blue-400 uppercase tracking-widest block">ACTIVE SCENARIO SIMULATION</span>
                  <h3 className="text-sm font-bold font-mono text-white uppercase">
                    SIMULATING: Emergency Protocol for Maya Johnson (Age 11)
                  </h3>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono bg-red-950 text-red-300 border border-red-800 px-2.5 py-0.5 rounded-full font-bold uppercase animate-pulse">
                  TRIGGER: {incident.activationMethod.replace(/_/g, " ")}
                </span>
                <button
                  onClick={() => setCurrentTab("simulator")}
                  className="text-[10px] font-mono bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 px-2.5 py-0.5 rounded-full font-bold uppercase transition-all cursor-pointer"
                >
                  ⚡ Control Simulation Ticks
                </button>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-[10.5px] font-mono text-slate-300">
              <div className="bg-slate-950/60 p-2.5 rounded-xl border border-slate-800">
                <span className="text-blue-400 block font-bold text-[9px] uppercase">1. TRIGGER SOURCE</span>
                <span className="text-white font-bold">{incident.activationMethod === "voice_phrase" ? "Voice Safeword 'Blue Folder'" : incident.activationMethod.replace(/_/g, " ")}</span>
              </div>
              <div className="bg-slate-950/60 p-2.5 rounded-xl border border-slate-800">
                <span className="text-blue-400 block font-bold text-[9px] uppercase">2. AI ASSESSMENT</span>
                <span className="text-amber-300 font-bold">{incident.aiRiskClassification?.toUpperCase() || "CRITICAL"} — High Arousal Panic</span>
              </div>
              <div className="bg-slate-950/60 p-2.5 rounded-xl border border-slate-800">
                <span className="text-blue-400 block font-bold text-[9px] uppercase">3. AWARENESS FAILOVER</span>
                <span className={incident.degradation?.phoneOff ? "text-red-400 font-bold" : "text-emerald-400 font-bold"}>
                  {incident.degradation?.phoneOff ? "BLE Tag Failover Active" : "GPS & Telemetry Active"}
                </span>
              </div>
              <div className="bg-slate-950/60 p-2.5 rounded-xl border border-slate-800">
                <span className="text-blue-400 block font-bold text-[9px] uppercase">4. RESPONDER STATUS</span>
                <span className="text-emerald-400 font-bold">
                  {incident.contacts.find(c => c.id === "contact_officer")?.respondStatus === "responding" ? "Campus Security Dispatched" : "Guardians Notified"}
                </span>
              </div>
            </div>
          </motion.div>
        ...
        <div className="bg-white border border-slate-200/80 p-6 rounded-3xl shadow-sm text-left space-y-5">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] font-mono font-extrabold text-slate-400 uppercase tracking-wider block">Protected Root Entity</span>
                <h2 className="text-base font-bold text-slate-900 tracking-tight flex items-center gap-2">
                  <span>Maya Johnson</span>
                  <span className="text-xs text-slate-500 font-normal">(Age 11)</span>
                </h2>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest block">Protection State</span>
                <span className="text-xs font-sans text-slate-600 font-medium">{globalDescription}</span>
              </div>
              <span className={`text-xs font-mono font-extrabold px-3.5 py-1.5 rounded-full uppercase tracking-wider border shadow-xs ${
                globalProtection === "Strong" ? "bg-emerald-100 text-emerald-850 border-emerald-300 animate-pulse" :
                globalProtection === "High" ? "bg-emerald-50 text-emerald-750 border-emerald-200" :
                globalProtection === "Reduced" ? "bg-amber-50 text-amber-800 border-amber-250" :
                globalProtection === "Limited" ? "bg-amber-100 text-amber-900 border-amber-300 animate-pulse" :
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
            ...
            <div className="md:col-span-4 space-y-2 bg-slate-50/80 p-4 rounded-2xl border border-slate-200/70">
              <div className="flex justify-between items-center text-xs font-mono">
                <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">System Integrity Meter</span>
                <span className="font-extrabold text-slate-800">{globalPercent}%</span>
              </div>
              <div className="h-2.5 w-full bg-slate-200/80 rounded-full overflow-hidden">
                <div 
                  className={`h-full rounded-full transition-all duration-500 ${globalPercentColor}`} 
            ...
            <div className="md:col-span-8 space-y-2">
              <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest block">Active Telemetry Sensor Feeds</span>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
                <div className={`p-2.5 rounded-xl border flex items-center gap-2 ${deg.phoneOff ? "bg-slate-50 text-slate-400 border-slate-200" : "bg-emerald-50/60 text-emerald-900 border-emerald-200 shadow-xs"}`}>
                  <span className={`w-2 h-2 rounded-full ${deg.phoneOff ? "bg-slate-300" : "bg-emerald-500 animate-pulse"}`} />
                  <span className="font-bold">Phone Hub</span>
                </div>
                <div className={`p-2.5 rounded-xl border flex items-center gap-2 ${deg.watchDisconnected || deg.phoneOff ? "bg-slate-50 text-slate-400 border-slate-200" : "bg-emerald-50/60 text-emerald-900 border-emerald-200 shadow-xs"}`}>
                  <span className={`w-2 h-2 rounded-full ${deg.watchDisconnected || deg.phoneOff ? "bg-slate-300" : "bg-emerald-500 animate-pulse"}`} />
                  <span className="font-bold">Apple Watch</span>
                </div>
                <div className={`p-2.5 rounded-xl border flex items-center gap-2 ${deg.tagDisconnected || deg.phoneOff ? "bg-slate-50 text-slate-400 border-slate-200" : "bg-emerald-50/60 text-emerald-900 border-emerald-200 shadow-xs"}`}>
                  <span className={`w-2 h-2 rounded-full ${deg.tagDisconnected || deg.phoneOff ? "bg-slate-300" : "bg-emerald-500 animate-pulse"}`} />
                  <span className="font-bold">BLE Tag</span>
                </div>
                <div className={`p-2.5 rounded-xl border flex items-center gap-2 ${deg.phoneOff ? "bg-slate-50 text-slate-400 border-slate-200" : "bg-emerald-50/60 text-emerald-900 border-emerald-200 shadow-xs"}`}>
                  <span className={`w-2 h-2 rounded-full ${deg.phoneOff ? "bg-slate-300" : "bg-emerald-500 animate-pulse"}`} />
                  <span className="font-bold">Glasses Audio</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* DEDICATED AI MULTIMODAL INTELLIGENCE SECTION */}
        <section className="pt-4">
          <div className="bg-gradient-to-br from-slate-900 via-slate-850 to-blue-950 p-6 md:p-8 rounded-3xl text-white shadow-lg space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold font-display tracking-tight text-white flex items-center gap-2">
                    <span>Gemini Multimodal Emergency Intelligence</span>
                    <span className="text-[10px] font-mono bg-blue-500/20 text-blue-300 border border-blue-400/30 px-2 py-0.5 rounded-full uppercase">Live AI</span>
                  </h3>
                  <p className="text-xs text-slate-300">Automated threat classification, live environmental audio processing & real-time translation</p>
                </div>
              </div>
            </div>
